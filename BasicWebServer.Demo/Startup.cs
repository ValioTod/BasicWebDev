...
namespace BasicWebServer.Demo
{
    public class Startup
    {
        ...
        {
            await DownloadAsTextFile(Startup.FileName, new string[] { "https://judge.softuni.org/", "https://softuni.org/" });

            var server = new HttpServer(router => router
        ...
        private static void DisplaySessionInfoAction(Request request, Response response)
        {
            ...
            if (sessionExists)
            {
                var currentDate = request.Session[Session.SessonCurrentDataKey];
        ...
        private static void AddCookiesAction(Request request, Response response)
        {
            ...
            if (requestHasCookies)
            {
                ...
                foreach (var cookie in request.Cookies)
                {
                    ...
                    cookieText.Append($"<td>{HttpUtility.HtmlEncode(cookie.Name)}</tb>");
                    cookieText.Append($"<td>{HttpUtility.HtmlEncode(cookie.Value)}</tb>");
                    cookieText.Append("</tr>");
                }
        ...
        private static async Task<string> DownloadWebContent(string url)
        {
        ...
        private static async Task DownloadAsTextFile(string fileName, string[] urls)
        {
            var downloads = new List<Task<string>>();

            foreach (var url in urls)
            {
                downloads.Add(DownloadWebContent(url));
            }
